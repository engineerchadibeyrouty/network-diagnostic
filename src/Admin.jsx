...
import {
  ...
  Eye, X, Trash2, Ban, ShieldCheck, KeyRound, ChevronDown, ChevronUp, Zap, Send, Ticket
} from "lucide-react"
...
export default function Admin({ onBack }) {
  ...
  const sendAdminReply = async () => {
    ...
    const res = await ticketAction("reply_ticket", {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      ...
      <div className="flex gap-2 mb-8 flex-wrap">
        {["overview", "users", "conversations", "tickets"].map(tab => (
          ...
          <div className="bg-gray-800 rounded-2xl p-6 mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
              <div>
                ...
                <p className="text-xl font-bold">${BUDGET_USD.toFixed(2)}</p>
              </div>
              <div>
            ...
            {tickets.map(t => (
              <div key={t.id} className="flex items-center justify-between p-3 bg-gray-700 rounded-xl hover:bg-gray-600 transition">
                <div className="flex-1 truncate">
                  ...
                  <p className="text-xs text-gray-400">
                    👤 {t.user_email} • {t.messages?.length || 0} messages • {new Date(t.updated_at).toLocaleString()}
                  </p>
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {selectedTicket.messages?.map((msg, i) => (
                <div key={i} className={`p-3 rounded-2xl max-w-lg ${
                  msg.role === "admin" ? "bg-blue-600 self-end ml-auto text-white" : "bg-gray-700 text-gray-100"
                }`}>
